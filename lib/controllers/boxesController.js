...
const { config, Honeybadger } = utils,
  { Measurement, Box, Sensor, Person } = models,
  ...
  Box.findOne({ _id: req._userParams.boxId })
    .then(function (box) {
      if (!box) {
        ...
        Person.findById(box.userID).then (function(person){
          var sbP = box.sensors.length+1;
          person.sbPoints = +person.sbPoints + +sbP;
          person.save();
        });
...
const postNewBox = function postNewBox (req, res, next) {
  ...
  Box.newFromRequest(req)
    .then(function (user) {
      ...
      Person.findById(req.payload._id).then(function (person) {
                Box.findOne({'name' :req.params.name}).then(function (boxUpdate) {

                    person.boxName.push(boxUpdate.name);
                    person.boxID.push(boxUpdate._id);
                    person.save();
                });
            }
        );
        ...
        Box.findOne({'name' :req.params.name}).then(function (boxUpdate) {
            Person.findById(req.payload._id).then(function (person) {
              boxUpdate.userName = person.firstname+" "+person.lastname;
              boxUpdate.userID = person._id;
              boxUpdate.save();
            });
        });
        res.send(201, user);
